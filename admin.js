...
async function getClasses() {
  ...
  const { data: sts } = await supabase.from('students').select('class_name');
...
async function renderOverview() {
  const [{ count: sc }, { count: lc }, { data: todayAlerts }, { data: recentLessons }, { data: recentAlerts }] = await Promise.all([
    supabase.from('students').select('*', { count: 'exact', head: true }),
    supabase.from('lessons').select('*', { count: 'exact', head: true }),
    supabase.from('alerts').select('*').gte('created_at', new Date().toISOString().split('T')[0]),
    supabase.from('lessons').select('id,name,class_name').order('created_at', { ascending: false }).limit(4),
    supabase.from('alerts').select('*').order('created_at', { ascending: false }).limit(4),
  ]);
  const { data: vids } = await supabase.from('lesson_videos').select('id', { count: 'exact' });
  const { data: docs } = await supabase.from('lesson_docs').select('id', { count: 'exact' });
...
document.getElementById('csSaveBtn').addEventListener('click', async () => {
  ...
  const { error } = await supabase.from('students').insert({ full_name:name, username, password, class_name:cls, active:true });
...
async function renderMiniStudents() {
  const { data: list } = await supabase.from('students').select('*').order('created_at', { ascending: false });
...
document.getElementById('addStudentSaveBtn').addEventListener('click', async () => {
  ...
  const { error } = await supabase.from('students').insert({ student_code:code, full_name:name, phone, username, password, class_name:cls, active:true });
  if (error) { err.textContent = error.message.includes('unique') ? 'Gmail đã tồn tại.' : error.message; return; }
...
let pendingLessonVideoFile = null;
let pendingLessonDocFile = null;
...
async function renderLessons() {
  ...
  for (const l of (list||[])) {
    const { count: vc } = await supabase.from('lesson_videos').select('*', { count:'exact', head:true }).eq('lesson_id', l.id);
    const { count: dc } = await supabase.from('lesson_docs').select('*', { count:'exact', head:true }).eq('lesson_id', l.id);
    ...
    row.innerHTML = `
      ...
      <div class="list-info">
        ...
        <div class="list-meta">
          ...
          <span>🎬 ${vc||0} video</span> • <span>📄 ${dc||0} tài liệu</span>
...
function openLessonModal(l = null) {
  ...
  populateClassFilters().then(() => {
    document.getElementById('lClassSelect').value = l ? (l.class_name || '') : '';
  });
...
document.getElementById('lSaveBtn').addEventListener('click', async () => {
  const name = document.getElementById('lNameInput').value.trim();
  ...
  if (editingLessonId) {
    await supabase.from('lessons').update({ name, class_name:cls, description:desc }).eq('id', editingLessonId);
  } else {
    await supabase.from('lessons').insert({ name, class_name:cls, description:desc });
  }
...
async function renderLessonVideos(lessonId) {
  const { data: vids } = await supabase.from('lesson_videos').select('*').eq('lesson_id', lessonId).order('created_at');
  ...
  (vids||[]).forEach(v => {
    const url = supabase.storage.from('lessons').getPublicUrl(v.storage_path).data.publicUrl;
    ...
    card.querySelector('.del-btn').addEventListener('click', async () => {
      await supabase.storage.from('lessons').remove([v.storage_path]);
      await supabase.from('lesson_videos').delete().eq('id', v.id);
...
async function renderLessonDocs(lessonId) {
  const { data: docs } = await supabase.from('lesson_docs').select('*').eq('lesson_id', lessonId).order('created_at');
  ...
  (docs||[]).forEach(d => {
    const url = supabase.storage.from('lessons').getPublicUrl(d.storage_path).data.publicUrl;
    const row = document.createElement('div');
    row.className = 'content-row clickable';
    ...
    row.querySelector('.btn-danger').addEventListener('click', async e => {
      e.stopPropagation();
      await supabase.storage.from('lessons').remove([d.storage_path]);
      await supabase.from('lesson_docs').delete().eq('id', d.id);
...
document.getElementById('lvSaveBtn').addEventListener('click', async () => {
  ...
  if (!title) return;
  const path = `videos/${currentLessonId}/${Date.now()}_${pendingLessonVideoFile.name}`;
  const { error: upErr } = await supabase.storage.from('lessons').upload(path, pendingLessonVideoFile);
  if (upErr) { alert('Lỗi upload: ' + upErr.message); return; }
  await supabase.from('lesson_videos').insert({ lesson_id: currentLessonId, title, file_name: pendingLessonVideoFile.name, storage_path: path });
  document.getElementById('lessonVideoModal').classList.remove('open');
  document.getElementById('lessonPreviewVideo').src = '';
  pendingLessonVideoFile = null;
...
document.getElementById('ldSaveBtn').addEventListener('click', async () => {
  ...
  const title = document.getElementById('ldTitleInput').value.trim();
  if (!title) return;
  const path = `docs/${currentLessonId}/${Date.now()}_${pendingLessonDocFile.name}`;
  const { error: upErr } = await supabase.storage.from('lessons').upload(path, pendingLessonDocFile);
  if (upErr) { alert('Lỗi upload: ' + upErr.message); return; }
  await supabase.from('lesson_docs').insert({ lesson_id: currentLessonId, title, file_name: pendingLessonDocFile.name, file_type: pendingLessonDocFile.type, storage_path: path });
  document.getElementById('lessonDocModal').classList.remove('open');
  pendingLessonDocFile = null;
...
async function renderClasses() {
  ...
  allNames.forEach(cls => {
    ...
    card.innerHTML = `
      ...
      <div style="flex:1"><div class="stat-num">${count}</div><div class="stat-label">${cls}</div></div>
      <div style="display:flex;flex-direction:column;gap:.25rem;align-self:flex-start">
        <button class="btn-sm" data-edit="${cls}">✏️</button>
        <button class="btn-sm btn-danger" data-del="${cls}">🗑</button>
      </div>`;
    ...
    card.querySelector('[data-del]').addEventListener('click', async e => {
      ...
      if (confirm(`Xóa lớp "${cls}"?`)) {
        await supabase.from('classes').delete().eq('name', cls);
        await supabase.from('students').update({ class_name: '' }).eq('class_name', cls);
...
async function openClassDetail(cls) {
  ...
  const { data: list } = await supabase.from('students').select('*').eq('class_name', cls);
...
document.getElementById('editClassSaveBtn').addEventListener('click', async () => {
  ...
  await supabase.from('classes').upsert({ name: newName });
  await supabase.from('classes').delete().eq('name', editingClassName);
  await supabase.from('students').update({ class_name: newName }).eq('class_name', editingClassName);
...
document.getElementById('addClassSaveBtn').addEventListener('click', async () => {
  ...
  const err  = document.getElementById('addClassError');
