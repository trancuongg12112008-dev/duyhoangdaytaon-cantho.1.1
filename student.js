...
async function loadMe() {
  const { data } = await supabase.from('students').select('class_name').eq('username', currentUser).single();
...
async function renderHome() {
  let query = supabase.from('lessons').select('id,name,class_name').order('created_at', { ascending: false }).limit(4);
  ...
  for (const l of list) {
    const [{ count: vc }, { count: dc }] = await Promise.all([
      supabase.from('lesson_videos').select('*', { count:'exact', head:true }).eq('lesson_id', l.id),
      supabase.from('lesson_docs').select('*', { count:'exact', head:true }).eq('lesson_id', l.id),
    ]);
...
async function openLessonDetail(id) {
  ...
  const { data: vids } = await supabase.from('lesson_videos').select('*').eq('lesson_id', id).order('created_at');
  ...
  const { data: docs } = await supabase.from('lesson_docs').select('*').eq('lesson_id', id).order('created_at');
